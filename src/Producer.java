...
public class Producer extends Thread implements Runnable{
    ...
    public Producer(Bank account, String threadName, int speed, int max, int count) { //Producer constructor to execute
    ...
    public void run() { //Method to run a thread and its functions
            try {
                ...
                account.Job(200, threadName, count); // Deposits money earned from job to Balance
                account.deposit(1 + generator.nextInt(max), threadName, count); //Using Producer to deposit to Balance
            }
